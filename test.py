...
data = data.drop(columns=data.columns[[1, 2]])  # Удаление 2-х первых ненужных столбцов
...
for i, row in data.iterrows():
    ...
    # Перебираем все часы, используя числовые индексы для столбцов с 0 до 23
    for hour in range(24):
        time = f"{hour:02d}:00"
        column_name = hour  # Название столбца — это сам час, т.е. 0, 1, 2, ..., 23

        # Получаем значение для этого часа (используем column_name вместо индекса)
        value = row[column_name]  # Здесь обращаемся по названию столбца (часу)
