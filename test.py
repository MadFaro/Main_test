...
import pandas as pd
from pywebio.output import put_html, put_text, put_button, put_row, put_column, put_datatable
from pywebio.platform import clear
...
async def dashboard_user(df_user, sdep, tab, fio, id):
    ...
    def create_login_chart(dates, login_counts):
        ...
        p.circle(x=dates, y=login_counts, size=8, color="blue")
    ...
    def create_multi_chart(dates, shop, game, box):
        p = figure(x_range=dates, title="Сервисы", plot_width=800, plot_height=400,
                   toolbar_location=None, tools="hover", tooltips="@x: @y")
    ...
    def create_box_chart(dates, quest, offer, mood):
        p = figure(x_range=dates, title="Бокс ОС", plot_width=800, plot_height=400,
                   toolbar_location=None, tools="hover", tooltips="@x: @y")
    ...
    def create_orders_cancellations_chart(dates, order_counts, cancellation_counts):
        p = figure(x_range=dates, title="Заказы", plot_width=800, plot_height=400,
                   toolbar_location=None, tools="hover", tooltips="@x: @y")
