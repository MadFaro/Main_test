def upsert_data_to_oracle_sl(rows):
    ...
    try:
        ...
        # Уникальный список threadid для удаления
        ...
        DECLARE
            ...
            CURSOR cur IS
                ...
                WHERE cth.threadid IN (SELECT COLUMN_VALUE FROM TABLE(CAST(:thread_ids AS SYS.ODCINUMBERLIST)))
                AND ct.offline_ = 0
        ...
        # Передача параметров PL/SQL блоку
        cursor.execute(plsql_block, thread_ids=unique_thread_ids)
        oracle_conn.commit()
