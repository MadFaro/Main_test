...
# Применяем только к числовым столбцам, исключая первый и второй (дата и день недели)
data.iloc[:, 2:] = data.iloc[:, 2:].applymap(lambda x: pd.to_numeric(x, errors='coerce')).fillna(0).astype(int)
