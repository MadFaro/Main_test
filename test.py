...
lead(int_create_date) over (partition by client_did order by int_create_date) as next_date
from tab_1
),
tab_3 as (
select client_did, deact_description, clnt_segment, int_create_date, next_date, dozvon,
case when next_date is not null then (next_date - int_create_date) else null end as day_diff
from tab_2
),
tab_4 as (
select client_did, deact_description, clnt_segment, int_create_date, next_date, day_diff, dozvon,
case when next_date is null then '0 повторений'
