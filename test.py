...
# Чтение данных из Excel, столбцы с 28 по 53
...
for i, row in data.iterrows():
    ...
    # Перебираем все часы, используя числовые индексы
    for hour in range(24):
        # Формируем строку для записи, используя числовой индекс для часов
        time = f"{hour:02d}:00"
        value = row[hour + 1]  # Обращаемся к столбцам по числовым индексам (с учетом смещения)
