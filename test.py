...
def on_add_button_click():
    ...
    # Проверяем, существует ли активная версия для выбранной платформы
    cursor.execute(f"SELECT VERSION FROM ANALYTICS.TOLOG_TEST1 WHERE PLATFORM = '{platform}' AND APP_ACT = 1")
    active_version = cursor.fetchone()
    
    if active_version:  # Если есть активная версия
        messagebox.showwarning("Add Version", f"Cannot add new version for {platform} without removing the active version ({active_version[0]}).")
    else:  # Если активной версии нет
