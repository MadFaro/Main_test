async def main_menu(sdep, tab, fio, id):
    ...
    product_cards.append(
        put_tabs([{'title': f"ТВОЙ МАГАЗИН МЕРЧА", 'content':[
                    put_column([
                    put_image(open('main_menu/img/logo1.png', 'rb').read(), width='100%', height="100%").style(
                        'border-radius: 15px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); transition: transform 0.2s ease-in-out;'),
                    ...
                    put_button("", onclick=lambda: noadmin(sdep, tab, fio, id), 
                            color='dark', outline=True).style('''
                                ...
                                border: none;
                            ''')
                ]}]).style('grid-column: span 1; grid-row: span 1; display: block; text-align:center; transition: transform 0.2s ease-in-out;')
    )
    product_cards.append(
        put_tabs([{'title': f"ГЕЙМИФИКАЦИЯ", 'content':[
                    put_column([
                    put_image(open('main_menu/img/logo2.png', 'rb').read(), width='100%', height="100%").style(
                        'border-radius: 15px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); transition: transform 0.2s ease-in-out;'),
                    ]).style('grid-template-rows:1fr'),
    ...
    product_cards.append(
        put_tabs([{'title': f"БОКС ОБРАТНОЙ СВЯЗИ", 'content':[
                    put_column([
                    put_image(open('main_menu/img/logo3.png', 'rb').read(), width='100%', height="100%").style(
                        'border-radius: 15px; box-shadow: 0 4px 8px rgba(0, 0, 0, 0.1); transition: transform 0.2s ease-in-out;'),
                    ]).style('grid-template-rows:1fr'),
