...
FROM (
    ...
    LEFT JOIN dm.contracts b 
        ON a.client_did = b.client_did 
        AND b.PRODUCT_CATEGORY_NM = 'Кредитная карта'
    WHERE TRUNC(a.REPORT_MONTH, 'mm') >= DATE'2022-01-01' 
        AND a.PRIOR_PRODUCT = 'Кредитная карта' 
        AND b.ACC_OPEN_DT IS NOT NULL
)
WHERE rn = 1;
