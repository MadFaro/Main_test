...
# Используем label_encoder только на обучающих данных, чтобы избежать утечки данных
...
model = Sequential([
    Dense(model_neiro_1, activation=model_f_act_1, input_shape=(X_train_tfidf.shape[1],)),
...
model.fit(X_train_tfidf, y_train_encoded, epochs=1000, batch_size=batch_s, verbose=1, callbacks=callbacks)
