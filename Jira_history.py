...
SET DATE_BEGIN_CRIF_FIX = DATE_IN_WORK_FIX - (
    ...
    END) / (24 * 60)
WHERE 
    EXTRACT(HOUR FROM (CAST(DATE_IN_WORK_FIX AS TIMESTAMP) - CAST(DATE_BEGIN_CRIF_FIX AS TIMESTAMP))) > 2
    AND EXTRACT(HOUR FROM (CAST(DATE_IN_WORK_FIX AS TIMESTAMP) - CAST(DATE_BEGIN_CRIF_FIX AS TIMESTAMP))) < 18
